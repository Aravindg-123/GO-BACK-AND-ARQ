...
(function () {
  ...
  // Inject minimal CSS to guarantee .hidden works even without external CSS
  (function ensureHiddenCSS(){
    if (!document.getElementById("gbn-inline-style")) {
      const st = document.createElement("style");
      st.id = "gbn-inline-style";
      st.textContent = `
        .hidden { display: none !important; }
        .packet { user-select: none; padding:4px 8px; border-radius:999px; background:#e9f7f4; border:1px solid #bfe8dd; color:#0b1e2b; font: 12px/1.2 ui-monospace, monospace; }
        .packet.ack { background:#e8efff; border-color:#c6d7ff; }
        .stat-card { background: rgba(255,255,255,.85); border: 1px solid rgba(0,0,0,.08); border-radius: 12px; padding: 10px; }
        .stat-label { font: 12px/1.2 ui-sans-serif, system-ui; color: #3e5566; }
        .stat-value { font: 18px/1.2 ui-sans-serif, system-ui; color: #0b1e2b; font-weight: 700; margin-top: 4px; }
      `;
  ...
  async function sendFrame(seq){
    ...
    badge.style.cssText="display:inline-block;margin:0 6px 6px 0;";
  ...
  function finish() {
    running = false;
    clearTimer();
    log("Simulation complete — composing summary…");
    ...
    statsWrap.querySelectorAll(".stat-label, .stat-value, h3, h4").forEach(el => {
      el.style.color = "#000";
    });
    ...
    const lbl = {
      ...
      textbook: "Textbook diagonals",
      animated: "Animated replay"
  ...
  function nodeCircle(x,y,t, txtColor="#0b1e2b"){
    const g=ns("g");
    const c=cir(x,y,6); c.setAttribute("fill","rgba(0,0,0,0)"); c.setAttribute("stroke","rgba(0,0,0,.35)");
    const tx=txt(x-26,y-10,txtColor,12,t);
    g.appendChild(c); g.appendChild(tx); return g;
  }
  function nodeRect(x,y,t, txtColor="#0b1e2b"){
    const g=ns("g");
    const r=rect(x-20,y-12,40,24,6); r.setAttribute("fill","rgba(0,0,0,.05)"); r.setAttribute("stroke","rgba(0,0,0,.2)");
    const tx=txt(x-15,y+4,txtColor,12,t);
    g.appendChild(r); g.appendChild(tx); return g;
  }

  // label that supports (x,y,text) or (x,y,text,color)
  function label(x,y,text,color="#0b1e2b"){ return txt(x,y,color,14,text,true); }
  function dash(l,i){
    ...
    l.style.animation=`drawline .9s ${i*0.12}s ease forwards`;
    l.parentNode.appendChild(styleOnce());
  }
  ...
  // --------- Diagram renderers (single correct versions) ---------
  function renderVertical(host, diag, rows, animated){
    ...
    // guides
    svg.appendChild(vline(L,30,h-30,"#000"));
    svg.appendChild(vline(R,30,h-30,"#000"));
    svg.appendChild(label(L-25,20,"Sender","#000"));
    svg.appendChild(label(R-35,20,"Receiver","#000"));

    // rows
    for(let i=0;i<rows;i++){
      const y=40+i*gap;
      svg.appendChild(nodeCircle(L,y,`#${i}`,"#000"));
      svg.appendChild(nodeCircle(R,y,`#${i}`,"#000"));
    }
    ...
    diag.frames.forEach(f=>{
      ...
      const ln=hline(L,y,R,y,f.delivered?"#00a3ad":"#ff6b6b",f.delivered?0:1);
      if(animated) dash(ln,idx++);
      svg.appendChild(ln);
    });
    // acks (place slightly above row line)
    diag.acks.forEach(a=>{
      const y=40+Math.max(0,a.seq)*gap-12;
      const ln=hline(R,y,L,y,"#2a6bff",a.delivered?0:1);
      if(animated) dash(ln,idx++);
      svg.appendChild(ln);
    });
  ...
  function renderTextbook(host, diag, rows, animated){
    ...
    for(let i=0;i<rows;i++){
      const y=40+i*gap;
      svg.appendChild(nodeRect(L,y,`#${i}`,"#000"));
      svg.appendChild(nodeRect(R,y,`#${i}`,"#000"));
    }

    let idx=0;
    diag.frames.forEach(f=>{
      const y=40+f.seq*gap;
      const ln=seg(L,y,R,y+16,f.delivered?"#00a3ad":"#ff6b6b",f.delivered?0:1);
      if(animated) dash(ln,idx++);
      svg.appendChild(ln);
    });
    diag.acks.forEach(a=>{
      const y=40+Math.max(0,a.seq)*gap-12;
      const ln=seg(R,y+16,L,y,"#2a6bff",a.delivered?0:1);
      if(animated) dash(ln,idx++);
      svg.appendChild(ln);
    });
  ...
  // --------- Summary dispatcher (fix for finish()) ---------
  function renderDiagram(host, diag, rows, mode) {
    const animated = (mode === "animated");
    if (mode === "vertical" || mode === "animated") {
      renderVertical(host, diag, rows, animated);
    } else if (mode === "textbook") {
      renderTextbook(host, diag, rows, animated);
    } else {
      renderVertical(host, diag, rows, false);
    }
  }
